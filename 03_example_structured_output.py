...
prompt_template_force_structure = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            """
			Your task is to process survey data and return a JSON array
			with objects/dicts that hold the following keys:
			"text": The text of the segment,
			"category": The category of the section. Possible values include
			but are not limited to: "NVBugs", "JIRA", "Nvidia Help",
			"Support", "Devices", "OS",
			"clarity": If the segments sentiment is Negative and the text is
			clear enough for a human to understand the pain point, mark this
			field as "Clear". Otherwise, mark it as "Unclear".
			"sentiment": The user sentiment of the segment text
			(Positive / Negative / Neutral),
			"summary": A summary of the segment text in less than 10 words,
			using words from the text and skipping connecting words
			if necessary.
			Instructions:
			1. Break the user survey into multiple segments based on the
			main topic.
			2. Popular topics include: NVBugs, JIRA, Nvidia Help,
			Support, Devices, OS.
			3. The topic can be from the popular topics list or another
			topic you define. If unsure, use the topic "Other".
			4. Note that Nvidia Help is the unified ticketing system
			for IT and HR.
			5. Note that Devices include laptops and mobile devices.
			Ensure your response is a JSON array which holds objects/dicts
			each comprised of the mentioned keys. Here is an example -
			[
				(
			...
			""",
            # Once again, the response must be ONLY the JSON array
            # with objects/dicts and nothing else.
            # """,
        ),
        (
            "user",
...
survey = (
    "User 1: To save time when changing devices (LT or DT), "
    "it would be really great to have a step-by-step email "
    "or tutorial reminding you how to transfer all your "
    "datas, including reinstalling softwares, without "
    ...
    prompt_template = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """
                Your task is to process survey data and return the
                survey after putting it in a structured format (JSON)
                Instructions:
                1. Break the user survey into multiple segments
                based on the main topic.
                2. Popular topics include: NVBugs, JIRA, Nvidia Help,
                Support, Devices, OS.
                3. The topic can be from the popular topics list or
                another topic you define. If unsure, use "Other".
                4. Note that Nvidia Help is the unified ticketing
                system for IT and HR.
                5. Note that Devices include laptops and mobile devices.
                """,
            ),
            (
